...
export default function SecondInputUser() {
  ...
  const handleSubmit = async (e: any) => {
    ...
    const res = await axios.post("/api/clientes/buscarEmpleo", CURP);
    ...
    for (let i = 0; i < res.data.length; i++) {
      let positionsClass = new claseUsuario();

      positionsClass.setNombre(res.data[i].idPuesto);
      positionsClass.setCURP(res.data[i].nombrePuesto);
